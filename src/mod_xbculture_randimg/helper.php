...
class modXbcultureRandimgHelper {
	...
	static function getItems($params) {
	    ...
	    if ($usefilms) {
	        ...
	        $query->select('f.id AS id, '.$datetype.' AS ddate, f.title, f.poster_img AS image, "film" AS com')
	        ->from('#__xbfilms AS f');
	        if ($reviewed) {
	    ...
	    if ($usebooks) {
	        $query = $db->getQuery(true);
	        $query->select('b.id AS id, '.$datetype.' AS ddate, b.title, b.cover_img AS image, "book" AS com')
	        ->from('#__xbbooks AS b');
	        if ($reviewed) {
